...
namespace ElarosProject.ViewModel
{
    public class AssessmentVM
    {
        ...
        public ObservableCollection<AssessmentModel> getSymptomByUser(string userID) //my update- sumaiya
        {
            ...
            var result = SymptomUpdate.Where(u => u.User == userID);
            foreach (var record in result.GroupBy(s => s.Symptom)) 
            {
                groupedSymptoms.Add((AssessmentModel)record); //check cast- may be causing issues when building project 
            }
